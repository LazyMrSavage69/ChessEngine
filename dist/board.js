...
//
// chess.js v1.x is ESM-only — imported here from the same CDN URL as
// main.js. The browser dedupes the module fetch.
...
export function createBoard(containerEl, config = {}) {
  ...
  return {
    ...
    /**
     * Fully restore the board to a FEN — board state, turn, castling rights,
     * en-passant square, half-move clock and full-move number. Used by the
     * snapshot-based Undo/Redo system in main.js.
     *
     * After calling this you should also feed the same FEN into your
     * authoritative chess.js instance (`chess.load(fen)`) so the main thread's
     * game state stays in sync. We deliberately do NOT auto-trigger an engine
     * reply — the player decides what to do next.
     */
    restoreFEN(fen) {
      renderToken++ // invalidate any in-flight animation callbacks
      lastMove = null
      currentArrows = null
      // Clear any lingering arrow SVG lines (defs stay).
      svgOverlay.querySelectorAll('line').forEach(el => el.remove())
      currentFen = fen
      clearSelection()
      renderPosition(fen)

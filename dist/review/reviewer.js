...
// Clamp mate-like scores into a large centipawn bucket so delta math stays sane.
function normalizeScoreCp(score) {
  if (Math.abs(score) >= MATE_SCORE - 1000) return score > 0 ? 10000 : -10000
  return score
}

function cpToPawnUnits(cp) {
  return Math.round((cp / 100) * 100) / 100
}

function buildExplanation(classification, delta, isBest, bestMoveSan) {
  const label = classification?.label || 'Good'
  if (classification?.key === 'BOOK') {
    return 'Book move — theory-approved opening choice.'
  }
  if (classification?.key === 'BRILLIANT') {
    return 'Brilliant sacrifice — the only winning idea in a sharp position.'
  }
  if (classification?.key === 'GREAT') {
    return 'Great move — a difficult, engine-level find in a critical moment.'
  }
  if (isBest) {
    return 'Best move — matches the engine’s top recommendation.'
  }
  if (delta <= 40) {
    return 'Playable move with only a small evaluation dip.'
  }
  if (delta <= 120) {
    return `Inaccuracy — a small slip compared to ${bestMoveSan || 'the best move'}.`
  }
  if (delta <= 300) {
    return `Mistake — significant loss compared to ${bestMoveSan || 'the best move'}.`
  }
  return `Blunder — major error compared to ${bestMoveSan || 'the best move'}.`
}

/**
 * Analyse a full game and return per-move annotations.
 * @param {{ fen: string, san: string, player: string }[]} history – `san` is
 *   the new canonical field (used to be `moveSan`); the legacy name is still
 *   accepted as a fallback so older saved sessions don't break review.
...
export function reviewGame(history, onProgress) {
  ...
  for (let i = 0; i < history.length; i++) {
    const { fen, player } = history[i]
    // Tolerate either field name to keep older callers working.
    const sanText = history[i].san || history[i].moveSan
    ...
    try {
      ...
      if (bookHit && bookHit.move === sanText) {
        const game = new Chess(fen)
        const played = game.move(sanText)
        annotations.push({
          moveSan: played?.san || sanText,
          bestMove: null,
          delta: 0,
          classification: book(),
          fen,
          ...
          depth: REVIEW_DEPTH,
          principalVariation: [],
          explanation: 'Book move — theory-approved opening choice.',
        })
      ...
      if (!played) {
        annotations.push({
          moveSan: sanText, bestMove: null, delta: 0,
          ...
          depth: REVIEW_DEPTH,
          principalVariation: [],
          explanation: 'Invalid move in history — skipped from analysis.',
        })
        if (onProgress) onProgress({ index: i, total: history.length })
        continue
      }
      ...
      const bestCp = normalizeScoreCp(bestScore)
      const actualCp = normalizeScoreCp(actualScore)
      const delta = Math.max(0, bestCp - actualCp)
      ...
      annotations.push({
        ...
        player,
        arrow,
        evaluationBefore: cpToPawnUnits(bestCp),
        evaluationAfter: cpToPawnUnits(actualCp),
        evaluationLoss: cpToPawnUnits(delta),
        accuracy: Math.round(100 * Math.exp(-0.00375 * delta)),
        depth: REVIEW_DEPTH,
        principalVariation: bestMove ? [bestMove.san] : [],
        explanation: buildExplanation(classification, delta, isBest, bestMove?.san),
      })
    } catch (err) {
      annotations.push({
        moveSan: sanText, bestMove: null, delta: 0,
        ...
        depth: REVIEW_DEPTH,
        principalVariation: [],
        explanation: 'Analysis error — move classified as best by default.',
      })
    }
